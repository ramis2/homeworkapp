// ===== HOMEWORK APP - FIXED VERSION =====
// Time limit now tracks ONLY active usage, not background time
...
(function() {
    ...
    const SAVE_INTERVAL = 30000; // Save usage every 30 seconds
    ...
    function checkIdleState() {
        ...
        if (timeSinceActivity > IDLE_TIMEOUT) {
            state.isIdle = true;
            console.log('User idle - pausing time tracking');
        }
    }
    ...
    function checkTimeLimit() {
        ...
        const totalUsed = state.usage.ms;

        console.log('Time used:', formatTime(totalUsed), '/', formatTime(limitMs));
    ...
    function startUsageTracking() {
        // Save usage to storage every 30 seconds
        state.usageSaveInterval = setInterval(() => {
            if (!state.isIdle && document.visibilityState === 'visible') {
                saveUsage();
            }
        }, SAVE_INTERVAL);
        ...
        state.trackingInterval = setInterval(() => {
            ...
            // 1. Page is visible
            // 2. User is not idle
            // 3. Time limit screen is NOT showing
            // 4. Parent dashboard is NOT showing
            // 5. Lock screen is NOT showing
