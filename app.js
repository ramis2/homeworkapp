// ===== HOMEWORK APP + FOCUS MODE / SOCIAL BLOCKER =====
...
(function() {
    ...
    function loadData() {
        ...
        if (usageSaved) state.usage = JSON.parse(usageSaved);
        ...
        const focusSaved = localStorage.getItem('homeworkFocus');
        if (focusSaved) {
            state.focusMode = JSON.parse(focusSaved);
            if (state.focusMode.active) {
                activateFocusMode(false); // Don't restart timer
            }
        }
    }
    ...
    function startUsageTracking() {
        ...
        state.trackingInterval = setInterval(() => {
            ...
            const limitScreenHidden = $('time-limit-screen').classList.contains('hidden');
            const parentDashHidden = $('parent-dashboard').classList.contains('hidden');
            const lockScreenHidden = $('lock-screen').classList.contains('hidden');

            if (pageVisible && !state.isIdle && limitScreenHidden && parentDashHidden && lockScreenHidden) {
                state.usage.ms += 1000;
    ...
    function setupFocusMode() {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (target && target.href) {
                ...
                const blockedDomains = [
                    ...
                    'x.com', 'snapchat.com', 'youtube.com', 'youtu.be',
                    'discord.com', 'discord.gg', 'reddit.com', 'pinterest.com',
                    'whatsapp.com', 'telegram.org', 'twitch.tv', 'roblox.com'
                ];

                const isBlocked = blockedDomains.some(domain => url.includes(domain));
                const isExternal = !url.includes(window.location.hostname);

                if (isBlocked || (state.focusMode.active && isExternal)) {
                    e.preventDefault();
                    e.stopPropagation();
                    showBlockWarning(isBlocked ? 'social' : 'external');
                    return false;
                }
            }
        }, true);
        ...
        window.open = function(url, target, features) {
            if (url) {
                ...
                const blockedDomains = ['instagram.com', 'facebook.com', 'tiktok.com', 'twitter.com', 'x.com', 'snapchat.com', 'youtube.com', 'youtu.be', 'discord.com', 'discord.gg', 'reddit.com', 'pinterest.com', 'whatsapp.com', 'telegram.org', 'twitch.tv', 'roblox.com'];
                if (blockedDomains.some(d => lowerUrl.includes(d)) || (state.focusMode.active && !lowerUrl.includes(window.location.hostname))) {
                    showBlockWarning(blockedDomains.some(d => lowerUrl.includes(d)) ? 'social' : 'external');
                    return null;
                }
            }
    ...
    function setupNavigationBlocker() {
        ...
        window.addEventListener('beforeunload', function(e) {
            if (state.focusMode.active) {
                e.preventDefault();
                e.returnValue = 'Focus Mode is ON. Are you sure you want to leave?';
    ...
    function showBlockWarning(type) {
        const messages = {
            social: '🚫 Social media is blocked during homework time!',
            external: '🔗 External links are blocked in Focus Mode!'
        };
        showNotification(messages[type] || 'Blocked!');
        if (navigator.vibrate) navigator.vibrate([100, 50, 100]);
    }

    function toggleFocusMode() {
        if (state.focusMode.active) {
            // Need PIN to turn OFF
            showFocusExitLock();
        } else {
    ...
    function activateFocusMode(startTimer) {
        ...
        // Add focus banner
        let banner = $('focus-banner');
        if (!banner) {
            banner = document.createElement('div');
            banner.id = 'focus-banner';
            banner.innerHTML = '🔒 FOCUS MODE ON - Social media blocked';
            document.body.insertBefore(banner, document.body.firstChild);
        }
        banner.classList.remove('hidden');

        showNotification('🔒 Focus Mode ON! Social media blocked.');
    }
    ...
    function addTask() {
        ...
        if (!text) { showNotification('Please enter a task!'); return; }
    ...
    function renderParentTaskList() {
        const completed = state.tasks.filter(t => t.done).sort((a, b) => new Date(b.completedAt) - new Date(a.completedAt)).slice(0, 20);
        ...
        container.innerHTML = completed.map(t => 
            '<div class="parent-task-item"><span>' + escapeHtml(t.text) + '</span><span class="pt-subject">' + SUBJECT_ICONS[t.subject] + ' ' + t.subject + '</span></div>'
        ).join('');
    }
    ...
    function renderWeeklyChart() {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const count = state.tasks.filter(t => t.done && t.completedAt && new Date(t.completedAt).toDateString() === dateStr).length;
            weekData.push({ day: DAYS[d.getDay()], count: count });
        }
        const maxCount = Math.max(...weekData.map(d => d.count), 1);
        container.innerHTML = weekData.map(d => {
            const height = Math.max((d.count / maxCount) * 100, 4);
            return '<div class="chart-bar ' + (d.count > 0 ? 'filled' : '') + '" style="height: ' + height + '%"><span class="chart-bar-value">' + d.count + '</span><span class="chart-bar-label">' + d.day + '</span></div>';
        }).join('');
    }
    ...
    function changePin() {
        ...
        if (newPin.length !== 4 || !/^\d{4}$/.test(newPin)) {
            showNotification('PIN must be 4 digits'); return;
        }
    ...
    function resetAllData() {
        ...
        saveData(); saveUsage(); saveFocusState();
        renderTasks(); renderStats(); renderParentTaskList(); renderWeeklyChart();
        hideParentDashboard();
        deactivateFocusMode();
    ...
    function setupInstallPrompt() {
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            ...
            if (banner && $('time-limit-screen').classList.contains('hidden')) {
                banner.classList.remove('hidden');
            }
        });
        ...
        const dismissBtn = $('dismiss-install');
        if (dismissBtn) {
    ...
    function setupEventListeners() {
        ...
        $('override-limit').addEventListener('click', () => { showParentLock(); window._limitOverride = true; });

        // Focus mode
        const focusBtn = $('focus-toggle');
        if (focusBtn) focusBtn.addEventListener('click', toggleFocusMode);

        const focusExitBtn = $('focus-exit-confirm');
        if (focusExitBtn) focusExitBtn.addEventListener('click', confirmFocusExit);

        const focusCancelBtn = $('focus-exit-cancel');
        if (focusCancelBtn) focusCancelBtn.addEventListener('click', hideFocusExitLock);

        const focusPin = $('focus-exit-pin');
        if (focusPin) focusPin.addEventListener('keypress', (e) => { if (e.key === 'Enter') confirmFocusExit(); });
    }
